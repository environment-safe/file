...
const canonicalLocationToPath = (location, os, username)=>{
    return (
...
export const parseUrl = (url)=>{
    ...
    if((pos = url.indexOf('://')) !== -1){
        result.parsed = new URL(url);
    }else{
        if(url[0] === '.') result.type = 'relative';
        else result.type = 'absolute';
        result.parsed = posix.parse(url);
        if(result.parsed.dir && result.parsed.dir[0] === '!'){
            const target = result.parsed.dir.substring(1);
            result.parsed.original = result.parsed.dir;
            result.parsed.known = target.toLowerCase();
            result.parsed.dir = canonicalLocationToPath(target.toLowerCase(), 'darwin', 'khrome');
        }
    }
    return result;
};

export const renderUrl = (meta)=>{
    return meta.parsed.toString();
};
